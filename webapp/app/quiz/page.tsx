                        ...
                        <div className="grid grid-cols-1 gap-1.5">
                            {[
                                ...
                                <button
                                    ...
                                    className={`w-full h-12 px-4 rounded-[14px] border text-left transition-all flex items-center justify-between active:scale-[0.98] ${paymentMethod === method.id
                                        ? "bg-white border-[#C497A0] shadow-sm"
                                        : "bg-white/50 border-gray-100 text-gray-400"
                                        }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs ${paymentMethod === method.id ? "bg-[#C497A0] text-white" : "bg-gray-100 text-gray-900"}`}>
                                            {method.icon}
                                        </div>
                                        <div className="flex flex-col">
                                            <span className={`text-[13px] font-bold leading-none ${paymentMethod === method.id ? "text-[#1a1a1a]" : "text-gray-400"}`}>
                                                {method.name}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        {recommendation.subscription_plans.map((plan, idx) => (
                            ...
                            >
                                ...
                                <div className="text-right">
                                    <div className="text-[14px] font-bold text-[#1a1a1a]">{Math.round(plan.price).toLocaleString()} <span className="text-[9px] font-bold text-[#94a3b8]">сум</span></div>
                                </div>
                            </button>
                        ))}
                    </div>
