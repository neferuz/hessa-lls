...
export function PeriodFilter({ onPeriodChange, isCompact }: PeriodFilterProps) {
  ...
  return (
    <div className="relative z-[50]" ref={pickerRef}>
      <div className="flex items-center gap-1.5 sm:gap-2">
        <button 
          ...
          className={cn(
            ...
            period === "Месяц" 
              ? "bg-white dark:bg-zinc-800 text-black dark:text-white shadow-[0_2px_8px_rgba(0,0,0,0.08)] border-transparent" 
      ...
      {isPickerOpen && (
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-[280px] rounded-[1.25rem] bg-white dark:bg-[#1c1c1e] p-4 space-y-4 border-0">
          ...
          {/* Range display simplified */}
          {(rangeStart || rangeEnd) && (
            <div className="flex items-center justify-center px-2 py-2 bg-black/5 dark:bg-white/5 rounded-xl">
               <Calendar className="size-3 text-muted-foreground/40 mr-2" />
               <span className="text-[12px] font-medium tabular-nums text-foreground/80">
                  {rangeStart ? `${String(rangeStart).padStart(2, "0")}.${String(selectedMonth + 1).padStart(2, "0")}.${selectedYear % 100}` : ".."}
                  {' — '}
                  {rangeEnd ? `${String(rangeEnd).padStart(2, "0")}.${String(selectedMonth + 1).padStart(2, "0")}.${selectedYear % 100}` : ".."}
               </span>
            </div>
          )}
